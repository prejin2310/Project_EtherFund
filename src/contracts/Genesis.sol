...
contract Genesis {
    ...
    uint excessAmount = 0;
    if (projects[id].raised + msg.value > projects[id].cost) {
        excessAmount = projects[id].raised + msg.value - projects[id].cost;
        (bool success, ) = msg.sender.call{value: excessAmount}("");
        require(success, "Transfer failed");
        require(false, "Entered amount exceeds required amount. Excess amount refunded.");
    }
